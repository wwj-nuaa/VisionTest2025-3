...
std::optional<double> safe_divide(double a, double b) {
    if (b == 0.0) return std::nullopt;/* _____ */
    return a / b;
}
// ===== 填空 1 结束 =====
...
double divide_or_default(double a, double b) {
    return safe_divide(a, b).value_or(-1.0);   /* _____ */
}
// ===== 填空 2 结束 =====
...
using IntOrStr = std::variant<int,std::string,double>;/* _____ */
...
std::string describe(const IntOrStr& v) {
    return std::visit(overloaded{/* _____ */
